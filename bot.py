...
# 2. بيانات Telegram من متغيرات البيئة لضمان الأمان والاستقرار
...
# استخدام StringSession للجلسة الشخصية لتجنب أخطاء الملفات على ريندر
        ...
        async for message in user_client.iter_messages(channel_entity, limit=50):
            if message.sender_id:
                try:
                    ...
                    info_str = f"• الاسم: {s_name}\n  - المعرف: {s_username}\n  - الآيدي: `{sender.id}`\n  - الصفة: ينشر في القناة (مشرف محتمل)"
...
async def main():
    print("🔥 جاري بدء تشغيل عميل المستخدم وعميل البوت...")
    await user_client.start()
    await bot_client.start(bot_token=BOT_TOKEN)
    print("🔥 تم تشغيل البوت وكشف المخالفات بنجاح 24/7!")
